...
public class InventoryManager
{
    ...
    private static List<Location> structureShopChestWildList = new ArrayList<>();
    private static Location gateMineralShopChestLoc;
    private static Location normalShopChestLoc;
    private static Location normalShopChestWildLoc;
    ...
    public InventoryManager()
    {
        ...
        structureShopChestWildList.add(new Location(Bukkit.getWorld(WorldManager.lobbyWorld), 0, -63, 1));
        structureShopChestWildList.add(new Location(Bukkit.getWorld(WorldManager.lobbyWorld), 2, -63, 1));
        structureShopChestWildList.add(new Location(Bukkit.getWorld(WorldManager.lobbyWorld), 4, -63, 1));
        ...
        normalShopChestWildLoc = new Location(Bukkit.getWorld(WorldManager.lobbyWorld), -4, -63, 1);
    }
    ...
    public static Inventory makeStructureShopWild(int num)
    {
        Component invName = Component.text("StructureShop_" + num);
        Inventory inv = Bukkit.createInventory(null, 54, invName);
        Chest chest1 = (Chest) structureShopChestWildList.get(num).getBlock().getState();
        Chest chest2 = (Chest) structureShopChestWildList.get(num).getBlock().getLocation().add(1, 0, 0).getBlock().getState();
        for(int i = 0; i < 27; i++)
        {
            ItemStack item = chest1.getBlockInventory().getItem(i);
            if(item != null && !item.getType().equals(Material.AIR))
            {
                List<Component> loreList = new ArrayList<>();
                loreList.add(Component.text("구매가: 100P").color(TextColor.color(68, 255, 13)).decorate(TextDecoration.BOLD).decoration(TextDecoration.ITALIC, false));
                loreList.add(Component.text("쉬프트 클릭하면 64개 씩 거래할 수 있습니다.").color(TextColor.color(255, 255, 255)).decorate(TextDecoration.BOLD).decoration(TextDecoration.ITALIC, false));
                item.lore(loreList);
                inv.setItem(i, item);
            }
    ...
    public static Inventory makeNormalShopWild()
    {
        Component invName = Component.text("NormalShop");
        Inventory inv = Bukkit.createInventory(null, 54, invName);
        Chest chest1 = (Chest) normalShopChestWildLoc.getBlock().getState();
        Chest chest2 = (Chest) normalShopChestWildLoc.getBlock().getLocation().add(1, 0, 0).getBlock().getState();
        for(int i = 0; i < 27; i++)
        {
            ItemStack item = chest1.getBlockInventory().getItem(i);
            if(item != null && !item.getType().equals(Material.AIR))
